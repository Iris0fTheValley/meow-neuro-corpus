...
2. Transcribe audio using `faster-whisper` to `.tsv`.
   ...
   ```
3. Sanitize the raw transcription (e.g. replace misspelt names).
4. Upload to the database at http://localhost:1337/upload. This requires a video_metadata.tsv file in the following format:
   ```
