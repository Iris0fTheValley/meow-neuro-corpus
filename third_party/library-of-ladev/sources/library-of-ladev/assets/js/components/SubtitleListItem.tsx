...
export interface SubtitleListItemRowProps {
...
type SubtitleListItemProps = SubtitleListItemRowProps & { index: number; style?: CSSProperties };

const SubtitleListItem = memo(function SubtitleListItem(
  props: SubtitleListItemProps
): ReactElement | null {
  const {
    index,
    video,
  ...
  if (!video || !video.subtitles) return null;
  ...
  const primaryText = (
    <Typography
      height={theatreMode ? '4.5rem' : { xs: '4.5rem', sm: '3rem' }}
      sx={{ overflowWrap: 'break-word', wordBreak: 'break-word', overflow: 'auto' }}
    >
      {text}
    </Typography>
  );
  const subtitleData: SubtitleWithVideo = { subtitleId, videoUrl: url, startTime, text, timestamp };
  return (
    <ListItem
      style={props.style}
      disablePadding
    ...
    >
      ...
      <Box
        ...
        alignItems="center"
        sx={{ gap: 2, pr: 2, display: theatreMode ? 'none' : { xs: 'none', sm: 'flex' } }}
      >
        <IconButton
          edge="end"
          aria-label="copy-text"
          title="Copy text to clipboard"
          onClick={(e) => handleClickCopy(e, text)}
        >
          <ArticleIcon />
        </IconButton>
        <IconButton
          ...
          onClick={(e) =>
            handleClickCopy(e, `https://www.youtube.com/watch?v=${url}&t=${startTime}s`)
          }
        >
          <LinkIcon />
        </IconButton>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <IconButton
              ...
              title={
                isBookmarked
                  ? lastUsedCollectionName
                    ? `Remove from ${lastUsedCollectionName}`
                    : 'Remove bookmark'
                  : lastUsedCollectionName
                  ? `Save to ${lastUsedCollectionName}`
                  : 'Bookmark'
              }
              onClick={(e) => {
                e.stopPropagation();
                onBookmarkToggle(subtitleData);
              }}
            >
            ...
            {onOpenBookmarkPicker ? (
              <IconButton
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onOpenBookmarkPicker(e.currentTarget, subtitleData);
                }}
                sx={{ p: 0.25 }}
              >
      ...
      <Box
        ...
        justifyContent="center"
        alignItems="center"
        sx={{ gap: 2, pr: 2, display: theatreMode ? 'flex' : { xs: 'flex', sm: 'none' } }}
      >
        <IconButton
          edge="end"
          aria-label="subtitle-options"
          title="More options"
          onClick={(e) => handleClickMobileSubtitleOption(e, subtitleData)}
        >
          <MoreVertIcon />
        </IconButton>
      </Box>
