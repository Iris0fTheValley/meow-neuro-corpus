...
export const makeBookmarkItemId = ({ subtitleId }: { subtitleId: number }): string =>
  String(subtitleId);
...
export const setLastUsedCollectionId = (id: string): void =>
...
export const addToCollection = (
  collectionId: string,
  { subtitleId, videoUrl, startTime }: AddToCollectionInput
): string | null => {
...
export const createCollection = (name: string): Collection => {
  ...
  if (trimmed.length > MAX_COLLECTION_NAME_LENGTH)
    throw new Error(`Collection name too long (max ${MAX_COLLECTION_NAME_LENGTH})`);
...
export const renameCollection = (id: string, name: string): void => {
  ...
  const finalName = uniqueName(
    trimmed,
    collections.filter((c) => c.id !== id).map((c) => c.name)
  );
  const next = collections.map((c) =>
    c.id === id ? { ...c, name: finalName, updatedAt: Date.now() } : c
  );
  setCollections(next);
};
...
export function validateImport(
...
): asserts parsed is ImportPayload {
  ...
  for (const item of p.items) {
    ...
    if (
      ...
      item.videoUrl.length > 64
    )
      throw new Error('Invalid item.videoUrl');
...
export const importCollection = (parsed: unknown, rawByteLength?: number): Collection => {
  ...
  const finalName = uniqueName(
    parsed.collection.name.trim() || 'Imported',
    collections.map((c) => c.name)
  );
  const collection: Collection = {
    id: uuid(),
...
export const isBackupKind = (parsed: unknown): boolean =>
  (parsed as { kind?: string } | null)?.kind === 'backup';
...
export const buildBackupView = (parsed: ImportPayload): BackupView => {
  ...
  const searchResult = Array.from(videoMap.values()).sort(
    (a, b) => (a.firstOrderIndex ?? 0) - (b.firstOrderIndex ?? 0)
  );
